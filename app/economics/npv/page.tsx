...
import { npv } from "@/lib/economics";

export default function NPVPage() {
  const [cashFlowStr, setCashFlowStr] = useState("-1000000, 300000, 400000, 350000, 300000");
  const [discountRateFraction, setDiscountRateFraction] = useState("0.1");
  const [result, setResult] = useState<number | null>(null);

  const handleCalculate = () => {
    const flows = cashFlowStr.split(",").map((s) => Number(s.trim()));
    const value = npv({ cashFlows: flows, discountRateFraction: Number(discountRateFraction) });
    setResult(value);
  };
  ...
  return (
    <CalculatorLayout
      title="NPV"
      description="Net present value of a cash flow stream."
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        <div className="rounded-xl border border-border bg-surface-elevated p-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <InputField label="Discount rate" name="r" value={discountRateFraction} onChange={setDiscountRateFraction} />
          </div>
          <button
            type="button"
        ...
        <div>
          {result != null && (
            <ResultPanel items={[{ label: "NPV", value: result.toFixed(2) }]} />
          )}
        </div>
      </div>
    </CalculatorLayout>
